...
- CI 发布：Windows 构建启用 Embedding 语义搜索（macOS 因 ort 无预编译二进制暂不支持）

## [0.3.3] - 2026-03-09

### 新增

- `--path` 参数改为可选，默认使用当前工作目录（CWD），MCP 配置不再必须手动指定项目路径

### 优化

- BM25 搜索结果去重：父子代码块行范围重叠时只保留更具体的子块，避免类定义和其方法同时出现
- 同文件结果限流：同一文件最多返回 3 个结果，保证搜索结果来源多样性
- 修复 `term_frequency` 的 token 匹配逻辑：从 `contains`（子串匹配）改为精确匹配，避免搜 `user` 误命中 `userService`、`userName` 等不相关 token
